...
kd = 0.01  # Reduce KD to prevent stopping at turns
...
while True:
    if cam.isRedLineDetected:
        ...
        if abs(curr_error) > error_threshold:
            ...
            speedDelta = min(MAX_SPEED / 2.5, abs(output))

            # Adjust PID Speed for right and left
            if curr_error > 0:
                # If error is positive, turn right
                speed_left = (MAX_SPEED / 2.5) + speedDelta
                speed_right = (MAX_SPEED / 2.5) - speedDelta
            else:
